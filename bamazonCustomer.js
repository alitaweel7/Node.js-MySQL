    ...
    connection.query('SELECT * FROM products', function (err, res) {
        for (var i = 0; i < res.length; i++) {
            console.log('Item Id: ' + res[i].item_id + ' '
                + ' Product: ' + res[i].product_name + ' ' + "DEPARTMENT: " + res[i].department_name + ' Price: $' + res[i].price + ' | ' + ' Available: ' + res[i].stock);
        }
        purchaseItem();
    })
    ...
    inquirer.prompt([{
        ...
        connection.query('SELECT * FROM products WHERE item_id = ?', [answer.item_id], function (err, res) {
            if (answer.stock_quantity > res[0].stock_quantity) {
                ...
                console.log('');
                var math = res[0].stock - parseInt(answer.stock_quantity);
                connection.query(`UPDATE products SET stock=${math} WHERE item_id=${answer.stock_quantity};`);
                quitBamazon();
